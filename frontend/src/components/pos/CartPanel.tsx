        ...
        {selectedCustomer ? (
          <div className="flex justify-between items-start p-4 bg-blue-50 border border-blue-200 rounded-lg">
            <div className="flex gap-3">
              ...
              <div>
                ...
                {(selectedCustomer.mobile || selectedCustomer.phone) && (
                  <div className="text-xs text-gray-600 mt-1">{selectedCustomer.mobile || selectedCustomer.phone}</div>
                )}
            ...
            <div className="flex gap-2 text-gray-500">
              <Edit2 size={14} className="cursor-pointer hover:text-blue-600 transition-colors" onClick={onCustomerClick} />
              <X size={14} className="cursor-pointer hover:text-red-600 transition-colors" onClick={onClearCustomer} />
            </div>
          </div>
        ) : (
          <div ref={searchContainerRef} className="relative">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2.5 bg-blue-50 border border-blue-300 rounded-lg text-sm text-gray-800 placeholder-blue-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              />
            </div>

            {/* Search Results Dropdown */}
            {showSearchResults && customerSearchText.trim().length >= 2 && (
              <div className="absolute left-0 right-0 top-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg z-30 max-h-[200px] overflow-y-auto">
                {searchResults.length > 0 ? (
                  searchResults.map((customer) => (
                    ...
                    >
                      <div className="text-sm font-medium text-gray-800">{customer.customer_name}</div>
                      <div className="text-xs text-gray-500">{customer.mobile || customer.phone}</div>
                    </div>
                  ))
                ) : !noMatchForPhone ? (
                ...
                {/* Quick Add Section — appears when phone number typed with no match */}
                {noMatchForPhone && (
          ...
          <div className="relative">
            ...
            {showDeliveryDropdown && (
              <div className="absolute left-0 right-0 top-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg z-10">
                ...
                <button
                  onClick={() => { onDeliverySelect('delivery'); onShowDeliveryChallanModal(); }}
