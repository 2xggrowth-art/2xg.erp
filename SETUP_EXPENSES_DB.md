# Expenses Module Database Setup

## Quick Setup Instructions

To enable the Expenses module, you need to create the database tables in Supabase.
...
1. Refresh your expenses page: http://localhost:3002/expenses
2. You should see:
   - Summary cards with expense totals
   - Category breakdown
   - Sample expenses (if you included the sample data)
...
curl http://localhost:5002/api/expenses/summary

# Test expenses by category
curl http://localhost:5002/api/expenses/by-category

# Test get all expenses
curl http://localhost:5002/api/expenses
```
...
1. **expense_categories** table - Stores expense category definitions
   - 10 default categories (Food, Fuel, Tea, Travel, etc.)

2. **expenses** table - Stores all expense records
   - Expense details (number, item, amount, description)
   - Payment details (mode, voucher number, file uploads)
   - Approval workflow (status, approver, timestamps)
   - Metadata (date, paid by, branch, etc.)

3. **Indexes** - For optimal query performance

4. **Sample Data** (optional) - 4 sample expenses for testing

## Troubleshooting

If you encounter errors:

1. **"relation already exists"** - Tables are already created, skip to Step 3
2. **"permission denied"** - Make sure you're logged in as the database owner
3. **API still returns 500** - Restart your backend server:
   ```bash
   cd backend
   npm run dev
   ```
...
- Add new expenses (coming soon)
- Approve/reject expenses (coming soon)
- Generate expense reports (coming soon)
