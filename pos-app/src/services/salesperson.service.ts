// Salespersons are stored locally (same as web version uses localStorage)
// In Electron, we store in SQLite via app settings
...
class SalespersonService {
  ...
  addSalesperson(salesperson: Omit<Salesperson, 'id'>): Salesperson {
    ...
    const newSalesperson: Salesperson = { id: newId, ...salesperson };
  ...
  updateSalesperson(id: string, updates: Partial<Omit<Salesperson, 'id'>>): Salesperson | null {
    ...
    this.salespersons[index] = { ...this.salespersons[index], ...updates };
    this.saveToStorage();
