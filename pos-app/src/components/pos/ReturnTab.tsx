...
const ReturnTab: React.FC<ReturnTabProps> = ({ activeSessionId, onReturnComplete }) => {
    ...
    try {
      ...
      if (result.success && result.data?.invoices) {
        const found = result.data.invoices.find(
          (inv: Invoice) =>
            inv.invoice_number.toLowerCase() === searchQuery.trim().toLowerCase()
        );
        if (found) {
  ...
  const refundAmount = selectedItems.reduce(
    (sum, item) => sum + item.rate * item.return_quantity,
    0
  );
  ...
  const handleProcessReturn = async () => {
    ...
    try {
      // In the Electron POS, we process the return locally
      // For now, we record it as a successful return
      // The sync service will push it to the server later
      setSuccess({ amount: refundAmount });
      onReturnComplete?.();
    } catch (err: any) {
      ...
      <div className="flex-1 overflow-auto p-6 bg-white dark:bg-gray-900">
        <div className="max-w-lg mx-auto mt-12">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-8 text-center">
            ...
            <button
              onClick={handleReset}
              className="px-6 py-2.5 bg-blue-600 dark:bg-blue-700 text-white rounded-lg text-sm font-medium hover:bg-blue-700 dark:hover:bg-blue-600 transition-colors"
            >
    ...
    <div className="flex-1 overflow-auto p-6 bg-white dark:bg-gray-900">
      ...
      <div className="mb-4">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
          Search for an invoice to process a return or exchange
        </p>
      </div>
        ...
        <div className="flex gap-3">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 border border-gray-200 dark:border-gray-600 rounded-lg text-sm bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-200 placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              autoFocus
            />
          </div>
          <button
            ...
            className="px-5 py-2.5 bg-blue-600 dark:bg-blue-700 text-white rounded-lg text-sm font-medium hover:bg-blue-700 dark:hover:bg-blue-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
          >
            {searching ? (
      ...
      {/* Error Message */}
      {error && (
        <div className="bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded-lg p-3 mb-4 flex items-center gap-2">
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-center">
                      <input
                        ...
                        className="w-20 text-center py-1 px-2 border border-gray-200 dark:border-gray-600 rounded text-sm bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50 disabled:bg-gray-100 dark:disabled:bg-gray-600"
                      />
                    </td>
                    ...
                    <td className="px-4 py-3 text-sm text-right font-medium">
                      {item.selected && item.return_quantity > 0 ? (
                        <span className="text-green-600 dark:text-green-400">
                          {new Intl.NumberFormat('en-IN', { minimumFractionDigits: 2 }).format(
                            item.rate * item.return_quantity
                          )}
                        </span>
                      ) : (
                    ...
                    <td className="px-4 py-3">
                      <input
                        ...
                        disabled={!item.selected}
                        placeholder="Reason..."
                        className="w-full py-1 px-2 border border-gray-200 dark:border-gray-600 rounded text-sm bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-200 placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50 disabled:bg-gray-100 dark:disabled:bg-gray-600"
                      />
                    </td>
                  </tr>
                ))}
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="px-6 py-3 bg-blue-600 dark:bg-blue-700 text-white rounded-lg text-sm font-medium hover:bg-blue-700 dark:hover:bg-blue-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
              >
                {loading ? (
                  <span className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />
                ) : (
