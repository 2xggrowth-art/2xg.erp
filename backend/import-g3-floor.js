/**
 * Import Ground Floor G3 Inventory from consolidated Excel
 */
...
async function main() {
  const excelPath = path.resolve('C:\\Users\\Admin\\Downloads\\G3_FLOOR_INVENTORY_CONSOLIDATED.xlsx');
  ...
  // Find or create bin "ground floor g3"
  let { data: bin } = await supabase
    .from('bin_locations')
    .select('id, bin_code, location_id')
    .ilike('bin_code', 'ground floor g3')
    .single();

  if (!bin) {
    console.log('Bin "ground floor g3" not found, creating...');
    const { data: newBin, error: binErr } = await supabase
      .from('bin_locations')
      .insert({ bin_code: 'ground floor g3', location_id: location.id, description: 'Ground Floor G3 Inventory', status: 'active' })
      .select()
      .single();
  ...
  const { data: bill, error: billErr } = await supabase
    ...
    .insert({
      ...
      vendor_name: 'Ground Floor G3 Inventory Import',
      bill_date: today,
      due_date: today,
      subtotal: 0,
      tax_amount: 0,
      ...
      notes: 'Auto-imported from Ground Floor G3 Inventory Excel',
    })
    .select()
    .single();
