/**
 * Import Ground Floor Inventory from consolidated Excel
 * - Creates items (auto SKU, cost_price=0, unit_price=0, advanced_tracking_type='serial')
 * - Creates a bill to add stock to "ground floor g1" bin at "bch" location
 * - Generates serial numbers for each item
 */
...
async function main() {
  ...
  const excelPath = path.resolve('C:\\Users\\Admin\\Downloads\\G_FLOOR_INVENTORY_CONSOLIDATED.xlsx');
  ...
  // 3. Find or create bin "ground floor g1" at this location
  let { data: bin } = await supabase
    .from('bin_locations')
    .select('id, bin_code, location_id')
    .ilike('bin_code', 'ground floor g1')
    .eq('location_id', location.id)
    .single();

  if (!bin) {
    // Check if bin exists at any location
    ...
    if (existingBin) {
      bin = existingBin;
      console.log(`Found bin "ground floor g1" at different location, using it.`);
    } else {
      console.log('Bin "ground floor g1" not found, creating...');
      const { data: newBin, error: binErr } = await supabase
        .from('bin_locations')
        .insert({ bin_code: 'ground floor g1', location_id: location.id, description: 'Ground Floor G1 Inventory', status: 'active' })
        .select()
        .single();
  ...
  for (const row of rows) {
    ...
    // Generate serial numbers: SKU/1, SKU/2, ...
    const serialNumbers = [];
    for (let i = 1; i <= qty; i++) {
  ...
  const { data: bill, error: billErr } = await supabase
    ...
    .insert({
      ...
      vendor_name: 'Ground Floor G1 Inventory Import',
      bill_date: today,
      due_date: today,
      subtotal: 0,
      tax_amount: 0,
      ...
      notes: 'Auto-imported from Ground Floor G1 Inventory Excel',
    })
    .select()
    .single();
  ...
  // 9. Create bill items, bin allocations, and serial numbers
  let itemsAdded = 0;
  for (const bi of billItems) {
    // Create bill item with serial numbers
